...
import { TourCategoryDetails, TourCategoryTourReference } from "@/types/tour-category-types";
...
interface TourCategoryReadOnlyDetailsProps {
...
export const TourCategoryReadOnlyDetails: React.FC<TourCategoryReadOnlyDetailsProps> = ({
  category,
  allTours,
  loadingTours,
  expandedSections,
  ...
  // Get tours not already in the category
  const availableTours = allTours.filter(
    t => !category.tours.some(existing => existing.tourId === t.tourId)
  );
  ...
  const primaryTour = category.tours.find(t => t.primaryCategory);
  const otherTours = category.tours.filter(t => !t.primaryCategory);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: theme.textSecondary }}>
                Manage tours in this category ({category.totalTours} total)
              </p>
            </div>
          </div>
          <ChevronDown
                    ...
                    {otherTours.map((tour) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => onSetPrimaryTour(tour.tourId)}
