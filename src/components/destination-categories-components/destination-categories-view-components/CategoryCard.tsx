// components/destinations-components/CategoryCard.tsx
"use client";
...
const CategoryCard: React.FC<CategoryCardProps> = ({ category }) => {
  ...
  return (
    ...
    >
      ...
      {/* Thumbnail Gallery */}
      {hasMultipleImages && (
        <div className="px-4 pt-3" style={{ borderBottom: `1px solid ${theme.border}` }}>
          <div className="flex space-x-2 overflow-x-auto pb-2">
            {images.slice(0, 5).map((image, index) => (
              ...
              >
                <img
                  ...
                  style={{
                    borderColor: currentImageIndex === index ? (category.color || theme.primary) : theme.border,
                  }}
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = PLACE_HOLDER_IMAGE;
                  }}
            ...
            {images.length > 5 && (
              <div className="flex items-center justify-center w-14 h-14 rounded-lg" style={{ backgroundColor: hexToRgba(theme.textSecondary, 0.1) }}>
        ...
        <div className="mb-3">
          <div className="flex items-center gap-2 mb-2">
            <div 
              ...
              style={{ background: `linear-gradient(135deg, ${hexToRgba(category.color || theme.primary, 0.2)}, ${hexToRgba(category.hoverColor || theme.accent, 0.1)})` }}
            >
              <Tag className="w-4 h-4" style={{ color: category.color || theme.primary }} />
            </div>
            <h3 
              className="text-xl font-bold line-clamp-1 transition-colors duration-200 cursor-pointer flex-1"
              style={{ color: theme.text }}
              onMouseEnter={(e) => { e.currentTarget.style.color = category.color || theme.primary; }}
          ...
          <div className="flex items-center gap-2 ml-10">
            ...
            />
            <span className="text-xs" style={{ color: theme.textSecondary }}>Brand Colors</span>
          </div>
        </div>

        {/* Description */}
        <p className="text-sm mb-4 line-clamp-3 flex-grow leading-relaxed" style={{ color: theme.textSecondary }}>
        ...
        >
          <div className="text-center">
            <div className="flex items-center justify-center mb-1">
              <div 
                ...
                style={{ background: `linear-gradient(135deg, ${hexToRgba(category.color || theme.accent, 0.1)}, ${hexToRgba(category.color || theme.accent, 0.05)})` }}
              >
                <ImageIcon className="w-3.5 h-3.5" style={{ color: category.color || theme.accent }} />
              </div>
            </div>
          ...
          <div className="text-center">
            <div className="flex items-center justify-center mb-1">
              ...
              >
                <Calendar className="w-3.5 h-3.5" style={{ color: theme.warning }} />
              </div>
            </div>
