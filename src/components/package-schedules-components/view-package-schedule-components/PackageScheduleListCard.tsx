// components/package-schedule-components/PackageScheduleListCard.tsx
"use client";
...
interface PackageScheduleListCardProps {
  schedule: PackageScheduleListItem;
}

const PackageScheduleListCard: React.FC<PackageScheduleListCardProps> = ({ schedule }) => {
        ...
        <motion.div variants={contentVariants} initial="hidden" animate="visible">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm" style={{ color: theme.textSecondary }}>{packageName}</span>
                </div>
                {tourScheduleName && (
                  <div className="flex items-center gap-1">
                    <MapPin className="w-4 h-4" style={{ color: theme.accent }} />
                    <span className="text-sm" style={{ color: theme.textSecondary }}>{tourScheduleName}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          >
            ...
            <div className="flex items-center">
              ...
              <div>
                <div className="text-xs" style={{ color: theme.textSecondary }}>Schedule ID</div>
                <div className="text-sm font-semibold" style={{ color: theme.text }}>#{schedule.packageScheduleId}</div>
              </div>
            </div>
          </motion.div>
          ...
          {startDate && endDate && (
            <motion.div variants={itemVariants} className="mb-4">
              <div className="flex items-center gap-2 text-sm" style={{ color: theme.textSecondary }}>
                ...
                <span>Schedule Period: {formatDate(startDate)} - {formatDate(endDate)}</span>
              </div>
            </motion.div>
          )}
