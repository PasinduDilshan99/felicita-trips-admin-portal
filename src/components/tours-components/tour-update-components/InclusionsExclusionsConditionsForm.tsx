// components/tours-components/update-tour-components/InclusionsExclusionsConditionsForm.tsx
"use client";
...
import { Inclusion, Exclusion, Condition } from "@/types/tour-types";
import {
  InclusionInput,
...
export const InclusionsExclusionsConditionsForm: React.FC<InclusionsExclusionsConditionsFormProps> = ({
  ...
  const isInclusionUpdated = (id: number) => updatedInclusions.some(u => u.inclusionId === id);
  const getInclusionUpdate = (id: number) => updatedInclusions.find(u => u.inclusionId === id);

  const isExclusionRemoved = (id: number) => removedExclusions.includes(id);
  const isExclusionUpdated = (id: number) => updatedExclusions.some(u => u.exclusionId === id);
  const getExclusionUpdate = (id: number) => updatedExclusions.find(u => u.exclusionId === id);

  const isConditionRemoved = (id: number) => removedConditions.includes(id);
  const isConditionUpdated = (id: number) => updatedConditions.some(u => u.conditionId === id);
  const getConditionUpdate = (id: number) => updatedConditions.find(u => u.conditionId === id);
