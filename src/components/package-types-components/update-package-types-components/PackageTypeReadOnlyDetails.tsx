...
import { PackageTypeDetails, PackageBasicDetail } from "@/types/package-type-types";
...
interface PackageTypeReadOnlyDetailsProps {
...
export const PackageTypeReadOnlyDetails: React.FC<PackageTypeReadOnlyDetailsProps> = ({
  type,
  expandedSections,
  onToggleSection,
  onRemoveImage,
    ...
    <div className="space-y-6">
      ...
      >
        <button
          ...
          style={{
            backgroundColor: expandedSections.has("packages") ? `${theme.primary}05` : "transparent",
            borderBottom: expandedSections.has("packages") ? `1px solid ${theme.border}` : "none",
          }}
        >
          ...
          <ChevronDown
            ...
            style={{ 
              transform: expandedSections.has("packages") ? "rotate(180deg)" : "none", 
              ...
              {primaryPackage && (
                <div className="mb-4">
                  ...
                  <div className="p-3 rounded-lg" style={{ backgroundColor: `${theme.warning}10`, border: `1px solid ${theme.warning}30` }}>
                    <div>
                      <p className="text-sm font-medium" style={{ color: theme.text }}>{primaryPackage.packageName}</p>
                      <div className="flex items-center gap-2 mt-1 text-xs" style={{ color: theme.textSecondary }}>
                        {primaryPackage.startDate && (
                          <>
                            <Calendar className="w-3 h-3" />
                            <span>{primaryPackage.startDate?.split("T")[0]} → {primaryPackage.endDate?.split("T")[0]}</span>
                          </>
                        )}
                      </div>
                      {primaryPackage.description && (
                        <p className="text-xs mt-1" style={{ color: theme.textSecondary }}>{primaryPackage.description}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2">
                    {otherPackages.map((pkg) => (
                      ...
                      >
                        <p className="text-sm font-medium" style={{ color: theme.text }}>{pkg.packageName}</p>
                        <div className="flex items-center gap-2 mt-1 text-xs" style={{ color: theme.textSecondary }}>
                          {pkg.startDate && (
                            <>
                              <Calendar className="w-3 h-3" />
                              <span>{pkg.startDate?.split("T")[0]} → {pkg.endDate?.split("T")[0]}</span>
                            </>
                          )}
                          {pkg.color && (
                            <>
                              <span className="w-2 h-2 rounded-full ml-2" style={{ backgroundColor: pkg.color }} />
                              <span>Color: {pkg.color}</span>
                            </>
                          )}
                        </div>
                        {pkg.description && (
                          <p className="text-xs mt-1" style={{ color: theme.textSecondary }}>{pkg.description}</p>
                        )}
                      </div>
                    ))}
                  </div>
