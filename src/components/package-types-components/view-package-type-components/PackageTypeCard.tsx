// components/package-types-components/view-package-type-components/PackageTypeCard.tsx
"use client";
...
const PackageTypeCard: React.FC<PackageTypeCardProps> = ({ packageType, onImageClick }) => {
  ...
  const description = getSafeString(packageType?.description, "");
  ...
  const getModalImages = (): ImageModalImage[] => {
    ...
    return images.map((img: PackageTypeImage, idx: number) => ({
      url: img?.imageUrl || "",
  ...
  if (!packageType || !packageType.typeId) return null;
        ...
        {images.length > 1 && (
          <div className="px-4 pt-4 pb-2" style={{ borderBottom: `1px solid ${theme.border}` }}>
            <div className="flex gap-2 overflow-x-auto pb-2">
              {images.map((image: PackageTypeImage, index: number) => (
                <motion.div
