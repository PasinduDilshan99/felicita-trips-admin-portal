...
export const TourInformation: React.FC<TourInformationProps> = ({ schedule }) => {
    ...
    >
      ...
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <InfoCard label="Tour Duration" value={`${schedule.tourDuration} days`} />
          ...
          <InfoCard label="Tour Status" value={schedule.tourStatus} highlight={schedule.tourStatus === "ACTIVE"} />
          <InfoCard label="Assign Message" value={schedule.assignMessage || "No message"} />
        </div>
        {schedule.tourDescription && (
          <div className="p-3 rounded-lg" style={{ backgroundColor: `${theme.border}10` }}>
            <p className="text-xs font-medium mb-1" style={{ color: theme.textSecondary }}>Tour Description</p>
            <p className="text-sm" style={{ color: theme.textSecondary }}>{schedule.tourDescription}</p>
          </div>
        )}
      </div>
    </motion.div>
