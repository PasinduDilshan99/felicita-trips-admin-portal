...
import { ActivityInformationProps } from "@/types/activity-schedule-types";
    ...
    >
      ...
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <InfoCard
            label="Duration"
            value={`${schedule.durationHours} hours`}
          />
          ...
          >
            ...
            <p className="text-sm" style={{ color: theme.text }}>
              {schedule.availableFrom?.slice(0, 5)} -{" "}
              {schedule.availableTo?.slice(0, 5)}
            </p>
          </div>
          <div
            className="p-3 rounded-lg"
          ...
          <InfoCard
            label="Local Price"
            value={formatPrice(schedule.priceLocal)}
            highlight
          />
          <InfoCard
            label="Foreigners Price"
            value={formatPrice(schedule.priceForeigners)}
            highlight
          />
          <InfoCard
            label="Activity Status"
            value={schedule.activityStatus}
            highlight={schedule.activityStatus === "ACTIVE"}
          />
        </div>
