// components/activity-schedule-components/ActivitySelector.tsx
"use client";

import React, { useState, useEffect, useRef } from "react";
import { Activity, Search, ChevronDown, Check, AlertCircle, Loader, Clock, MapPin, DollarSign, Users, Calendar, RefreshCw } from "lucide-react";
...
interface ActivitySelectorProps {
  ...
  onActivitySelect: (activityId: number, activityDetails?: ActivityType) => void;
...
export const ActivitySelector: React.FC<ActivitySelectorProps> = ({
  ...
  const [activityDetails, setActivityDetails] = useState<ActivityType | null>(null);
  ...
  const fetchActivities = async () => {
    try {
      ...
      console.log("getActivityIdsAndNames method:", ActivityService.getActivityIdsAndNames);
      ...
      if (response.code === 200 && response.data) {
        ...
        if (selectedActivityId && selectedActivityId > 0) {
          const preSelected = data.find((a) => a.activityId === selectedActivityId);
          if (preSelected) {
      ...
      setApiError(err instanceof Error ? err.message : "Error loading activities. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    console.log("ActivitySelector mounted, calling fetchActivities");
    fetchActivities();
              ...
              >
                {loading ? (
                  ...
                  filteredActivities.map((activity) => {
                    const isSelected = currentSelectedId === activity.activityId;
                    return (
                      <button
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 pt-3 border-t" style={{ borderColor: theme.border }}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-xs" style={{ color: theme.textSecondary }}>Local Price</p>
                              <p className="text-xs font-medium" style={{ color: theme.text }}>
                          ...
                          <div className="flex items-center gap-2">
                            <DollarSign className="w-3.5 h-3.5 flex-shrink-0" style={{ color: theme.warning }} />
                            <div>
                              <p className="text-xs" style={{ color: theme.textSecondary }}>Foreigner Price</p>
                              <p className="text-xs font-medium" style={{ color: theme.text }}>
                                {formatCurrency(activityDetails.price_foreigners)}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs font-medium" style={{ color: theme.text }}>
                                {activityDetails.min_participate} - {activityDetails.max_participate} persons
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2 col-span-2">
                            ...
                            <div>
                              <p className="text-xs" style={{ color: theme.textSecondary }}>Available Time</p>
                              <p className="text-xs font-medium" style={{ color: theme.text }}>
                        ...
                        {activityDetails.activities_category && activityDetails.activities_category.length > 0 && (
                          <div className="pt-2 border-t" style={{ borderColor: theme.border }}>
                            <p className="text-xs font-medium mb-2" style={{ color: theme.textSecondary }}>
                              Categories:
                            </p>
                            <div className="flex flex-wrap gap-1">
                              {activityDetails.activities_category.map((category) => (
                                <span
                                  key={category.id}
                                  className="text-xs px-2 py-0.5 rounded-full"
                                  style={{
                                    backgroundColor: category.is_primary ? `${theme.primary}20` : `${theme.primary}10`,
                                    color: theme.primary,
                                  }}
                                >
                                  {category.name}
