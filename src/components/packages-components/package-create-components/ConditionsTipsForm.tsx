...
import { ConditionsTipsFormProps } from "@/types/package-types";
...
export const ConditionsTipsForm: React.FC<ConditionsTipsFormProps> = ({
  ...
  const removeCondition = (index: number) =>
    onConditionsChange(
      conditions
        .filter((_, i) => i !== index)
        .map((cond, idx) => ({ ...cond, displayOrder: idx })),
    );
  const addTravelTip = () => {
  ...
  const removeTravelTip = (index: number) =>
    onTravelTipsChange(
      travelTips
        .filter((_, i) => i !== index)
        .map((tip, idx) => ({ ...tip, displayOrder: idx })),
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: theme.textSecondary }}>
              Terms, conditions, and helpful tips
            </p>
          </div>
        </div>
        <ChevronDown
