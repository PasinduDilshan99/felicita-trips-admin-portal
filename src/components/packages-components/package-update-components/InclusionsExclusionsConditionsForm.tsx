// components/packages-components/update-package-components/InclusionsExclusionsConditionsForm.tsx
"use client";
...
import { PackageInclusionResponse, PackageExclusionResponse, PackageConditionResponse } from "@/types/package-types";
import { Inclusion, UpdateInclusionRequest, Exclusion, UpdateExclusionRequest, Condition, UpdateConditionRequest } from "@/types/package-types";
...
export const InclusionsExclusionsConditionsForm: React.FC<InclusionsExclusionsConditionsFormProps> = ({
  inclusions,
  exclusions,
  conditions,
  addedInclusions,
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(["inclusions", "exclusions", "conditions"]));
  ...
  const isInclusionUpdated = (id: number) => updatedInclusions.some(u => u.packageInclusionId === id);
  const getInclusionUpdate = (id: number) => updatedInclusions.find(u => u.packageInclusionId === id);

  const isExclusionRemoved = (id: number) => removedExclusions.includes(id);
  const isExclusionUpdated = (id: number) => updatedExclusions.some(u => u.packageExclusionId === id);
  const getExclusionUpdate = (id: number) => updatedExclusions.find(u => u.packageExclusionId === id);

  const isConditionRemoved = (id: number) => removedConditions.includes(id);
  const isConditionUpdated = (id: number) => updatedConditions.some(u => u.packageConditionId === id);
  const getConditionUpdate = (id: number) => updatedConditions.find(u => u.packageConditionId === id);
  ...
  const renderItem = (
    ...
    const currentText = update?.inclusionText || update?.exclusionText || update?.conditionText || item.description;
    ...
    return (
      ...
      >
        ...
        <div className="flex-1">
          <p className="text-sm" style={{ color: isActive ? theme.text : theme.textSecondary }}>
            {currentText}
          </p>
          {isUpdated && (
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={() => onUpdate(item.id, currentText, isActive ? "INACTIVE" : "ACTIVE")}
      ...
      <div className="px-4 sm:px-6 py-5 space-y-6">
        ...
        <div>
          ...
          >
            ...
            <ChevronDown
              ...
              style={{ transform: expandedSections.has("inclusions") ? "rotate(180deg)" : "none", color: theme.textSecondary }}
            />
          </button>
          ...
          {expandedSections.has("inclusions") && (
            <motion.div variants={sectionVariants} initial="hidden" animate="visible" className="mt-4">
              <AnimatePresence>
                {showInclusionForm && (
                  <motion.div variants={formVariants} initial="hidden" animate="visible" exit="exit" className="mb-4 p-3 rounded-lg" style={{ backgroundColor: `${theme.success}10` }}>
                    <textarea
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setShowInclusionForm(false)} className="px-3 py-1.5 rounded-lg text-sm" style={{ backgroundColor: theme.background, border: `1px solid ${theme.border}`, color: theme.textSecondary }}>Cancel</button>
                      <button onClick={handleAddInclusion} className="px-3 py-1.5 rounded-lg text-sm text-white" style={{ backgroundColor: theme.success }}>Add</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="space-y-2">
                {inclusions.map((inc) =>
                  renderItem(
                    ...
                    getInclusionUpdate,
                    (id, text, status) => onUpdateInclusion(id, text, inc.displayOrder, status),
                    onRemoveInclusion,
                    <CheckCircle className="w-4 h-4" style={{ color: theme.success }} />,
                    theme.success
                  )
              ...
              <button onClick={() => setShowInclusionForm(true)} className="mt-3 flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-all" style={{ backgroundColor: `${theme.success}10`, color: theme.success }}>
        ...
        <div>
          ...
          >
            ...
            <ChevronDown
              className="w-4 h-4 transition-transform"
              style={{ transform: expandedSections.has("exclusions") ? "rotate(180deg)" : "none", color: theme.textSecondary }}
            />
          </button>
          ...
          {expandedSections.has("exclusions") && (
            <motion.div variants={sectionVariants} initial="hidden" animate="visible" className="mt-4">
              <AnimatePresence>
                {showExclusionForm && (
                  <motion.div variants={formVariants} initial="hidden" animate="visible" exit="exit" className="mb-4 p-3 rounded-lg" style={{ backgroundColor: `${theme.error}10` }}>
                    <textarea
                    ...
                    />
                    <div className="flex gap-2">
                      <button onClick={() => setShowExclusionForm(false)} className="px-3 py-1.5 rounded-lg text-sm" style={{ backgroundColor: theme.background, border: `1px solid ${theme.border}`, color: theme.textSecondary }}>Cancel</button>
                      <button onClick={handleAddExclusion} className="px-3 py-1.5 rounded-lg text-sm text-white" style={{ backgroundColor: theme.error }}>Add</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="space-y-2">
                {exclusions.map((exc) =>
                  renderItem(
                    ...
                    getExclusionUpdate,
                    (id, text, status) => onUpdateExclusion(id, text, exc.displayOrder, status),
                    onRemoveExclusion,
                    <XCircle className="w-4 h-4" style={{ color: theme.error }} />,
                    theme.error
                  )
              ...
              <button onClick={() => setShowExclusionForm(true)} className="mt-3 flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-all" style={{ backgroundColor: `${theme.error}10`, color: theme.error }}>
        ...
        <div>
          ...
          >
            ...
            <ChevronDown
              className="w-4 h-4 transition-transform"
              style={{ transform: expandedSections.has("conditions") ? "rotate(180deg)" : "none", color: theme.textSecondary }}
            />
          </button>
          ...
          {expandedSections.has("conditions") && (
            <motion.div variants={sectionVariants} initial="hidden" animate="visible" className="mt-4">
              <AnimatePresence>
                {showConditionForm && (
                  <motion.div variants={formVariants} initial="hidden" animate="visible" exit="exit" className="mb-4 p-3 rounded-lg" style={{ backgroundColor: `${theme.warning}10` }}>
                    <textarea
                    ...
                    />
                    <div className="flex gap-2">
                      <button onClick={() => setShowConditionForm(false)} className="px-3 py-1.5 rounded-lg text-sm" style={{ backgroundColor: theme.background, border: `1px solid ${theme.border}`, color: theme.textSecondary }}>Cancel</button>
                      <button onClick={handleAddCondition} className="px-3 py-1.5 rounded-lg text-sm text-white" style={{ backgroundColor: theme.warning }}>Add</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="space-y-2">
                {conditions.map((cond) =>
                  renderItem(
                    ...
                    getConditionUpdate,
                    (id, text, status) => onUpdateCondition(id, text, cond.displayOrder, status),
                    onRemoveCondition,
                    <AlertTriangle className="w-4 h-4" style={{ color: theme.warning }} />,
                    theme.warning
                  )
              ...
              <button onClick={() => setShowConditionForm(true)} className="mt-3 flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-all" style={{ backgroundColor: `${theme.warning}10`, color: theme.warning }}>
