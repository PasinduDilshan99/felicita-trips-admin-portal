...
import { Star, Plus, ChevronDown, Edit2, Trash2, Info } from "lucide-react";
...
import { cardVariants, featureVariants, formVariants } from "@/app/animations/variants";
...
export const FeaturesManagement: React.FC<FeaturesManagementProps> = ({
  ...
  const [editingFeature, setEditingFeature] =
    useState<PackageFeatureResponse | null>(null);
  ...
  const isFeatureUpdated = (id: number) =>
    updatedFeatures.some((u) => u.featureId === id);
  const getFeatureUpdate = (id: number) =>
    updatedFeatures.find((u) => u.featureId === id);

  const handleAddFeature = () => {
    if (
      !newFeatureData.featureName.trim() ||
      !newFeatureData.featureValue.trim()
    ) {
  ...
  const handleUpdateFeature = () => {
    ...
    if (
      !editFeatureData.featureName.trim() ||
      !editFeatureData.featureValue.trim()
    ) {
      alert("Feature name and value are required");
      return;
    }
                      ...
                      <div className="flex flex-wrap gap-2 mb-2">
                        {PACKAGE_UPDATE_PRESET_COLORS.map((color) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setNewFeatureData({ ...newFeatureData, color })
                            }
                            className="w-8 h-8 rounded-full border-2 transition-all"
                            style={{
                              backgroundColor: color,
                              borderColor:
                                newFeatureData.color === color
                                  ? theme.text
                                  : "transparent",
                              boxShadow:
                                newFeatureData.color === color
                        ...
                        {PACKAGE_UPDATE_PRESET_COLORS.map((color) => (
                          <button
                            ...
                            style={{
                              backgroundColor: color,
                              borderColor:
                                newFeatureData.hoverColor === color
                                  ? theme.text
                                  : "transparent",
                              boxShadow:
                                newFeatureData.hoverColor === color
              ...
              <div className="space-y-2">
                {addedFeatures.map((feature, idx) => (
                  <div
                    ...
                    style={{
                      backgroundColor: hexToRgba(feature.color, 0.08),
                      border: `1px dashed ${feature.color}`,
                    }}
                  >
                    <Star
