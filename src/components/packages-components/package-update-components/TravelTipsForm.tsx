// components/packages-components/update-package-components/TravelTipsForm.tsx
"use client";

import React, { useState } from "react";
import { Lightbulb, Plus, X, ChevronDown, Trash2, Edit2, CheckCircle } from "lucide-react";
import { motion, AnimatePresence, type Variants } from "framer-motion";
import { PackageTravelTipResponse } from "@/types/package-types";
import { TravelTipRequest, UpdateTravelTipRequest } from "@/types/package-types";
...
export const TravelTipsForm: React.FC<TravelTipsFormProps> = ({
  ...
  const [editingTip, setEditingTip] = useState<PackageTravelTipResponse | null>(null);
  ...
  const isTipUpdated = (id: number) => updatedTravelTips.some(u => u.packageTipId === id);
  const getTipUpdate = (id: number) => updatedTravelTips.find(u => u.packageTipId === id);
  ...
  const handleUpdateTip = () => {
    ...
    onUpdateTravelTip(editingTip.id, editTipData.title, editTipData.description, editingTip.displayOrder, editTipData.status);
    setEditingTip(null);
  };
        ...
        <div className="px-4 sm:px-6 py-5">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setEditTipData({ ...editTipData, title: e.target.value })}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => setEditTipData({ ...editTipData, description: e.target.value })}
                        rows={3}
                    ...
                    <div>
                      ...
                      <div className="flex gap-3">
                        <button
                          onClick={() => setEditTipData({ ...editTipData, status: "ACTIVE" })}
                          className="flex-1 px-3 py-2 rounded-lg border-2 text-sm flex items-center justify-center gap-2"
                          style={{
                            backgroundColor: editTipData.status === "ACTIVE" ? `${theme.success}10` : theme.background,
                            borderColor: editTipData.status === "ACTIVE" ? theme.success : theme.border,
                            color: editTipData.status === "ACTIVE" ? theme.success : theme.text,
                          }}
                        >
                        ...
                        <button
                          onClick={() => setEditTipData({ ...editTipData, status: "INACTIVE" })}
                          className="flex-1 px-3 py-2 rounded-lg border-2 text-sm flex items-center justify-center gap-2"
                          style={{
                            backgroundColor: editTipData.status === "INACTIVE" ? `${theme.error}10` : theme.background,
                            borderColor: editTipData.status === "INACTIVE" ? theme.error : theme.border,
                            color: editTipData.status === "INACTIVE" ? theme.error : theme.text,
                          }}
                        >
                          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: theme.error }} />
                          Inactive
                        </button>
                      </div>
                    </div>
          ...
          {travelTips.length === 0 && addedTravelTips.length === 0 && (
            <div className="text-center py-8">
              <Lightbulb className="w-12 h-12 mx-auto mb-3 opacity-30" style={{ color: theme.textSecondary }} />
              <p className="text-sm" style={{ color: theme.textSecondary }}>
