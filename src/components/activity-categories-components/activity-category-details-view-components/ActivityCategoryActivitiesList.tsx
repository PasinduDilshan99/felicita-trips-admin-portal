// components/activity-categories-components/activity-category-details-view-components/ActivityCategoryActivitiesList.tsx
"use client";

import React, { useState } from "react";
import { Activity, Star, ChevronRight, ChevronDown, ChevronUp } from "lucide-react";
import { useTheme } from "@/contexts/ThemeContext";
import { PrimaryActivity, OtherActivity } from "@/types/activity-category-types";

interface ActivityCategoryActivitiesListProps {
...
export const ActivityCategoryActivitiesList: React.FC<ActivityCategoryActivitiesListProps> = ({
  primaryActivities,
  otherActivities,
  categoryColor,
  onViewActivity,
  ...
  const visiblePrimary = showAllPrimary ? primaryActivities : primaryActivities.slice(0, 5);
  const visibleOther = showAllOther ? otherActivities : otherActivities.slice(0, 5);
      ...
      >
        ...
        >
          <div className="flex items-center gap-2">
            <Activity className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: categoryColor }} />
            ...
            <div className="space-y-2">
              {visiblePrimary.map((activity) => (
                <button
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.backgroundColor = hexToRgba(theme.warning, 0.1);
                    e.currentTarget.style.transform = "translateX(4px)";
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.backgroundColor = hexToRgba(theme.warning, 0.05);
                ...
                >
                  ...
                  <ChevronRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200" style={{ color: theme.warning }} />
                </button>
              ))}
            </div>
            {hasMorePrimary && (
            ...
            <div className="space-y-2">
              {visibleOther.map((activity) => (
                <button
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.backgroundColor = hexToRgba(categoryColor, 0.08);
                    e.currentTarget.style.transform = "translateX(4px)";
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.backgroundColor = hexToRgba(categoryColor, 0.03);
                    e.currentTarget.style.transform = "translateX(0)";
                  }}
                >
                  ...
                  <ChevronRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200" style={{ color: categoryColor }} />
                </button>
              ))}
            </div>
            {hasMoreOther && (
