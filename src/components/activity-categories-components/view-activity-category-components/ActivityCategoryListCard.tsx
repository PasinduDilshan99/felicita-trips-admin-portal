// components/activity-category-components/ActivityCategoryListCard.tsx
"use client";
...
interface ActivityCategoryListCardProps {
  category: ActivityCategory;
  onImageClick?: (imageIndex: number) => void;
}

const ActivityCategoryListCard: React.FC<ActivityCategoryListCardProps> = ({ category, onImageClick }) => {
  ...
  const description = getSafeString(category?.categoryDescription, "No description available");
  ...
  const getModalImages = (): ImageModalImage[] => {
    ...
    return images.map((img: ActivityCategoryImage, idx: number) => ({
      url: img?.imageUrl || "",
          ...
          <motion.div variants={contentVariants} initial="hidden" animate="visible" className="lg:w-3/5 xl:w-2/3 p-5 sm:p-6">
            ...
            <motion.div variants={itemVariants} className="mb-4">
              <div className="flex items-start justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm" style={{ color: theme.textSecondary }}>ID: {category.categoryId}</span>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center">
                <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3" style={{ background: hexToRgba(color, 0.1) }}>
                  <Activity className="w-5 h-5" style={{ color }} />
                </div>
                <div>
                  <div className="text-xs" style={{ color: theme.textSecondary }}>Activities</div>
                  <div className="text-sm font-semibold" style={{ color: theme.text }}>{numberOfActivities}</div>
                </div>
              </div>
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-xs" style={{ color: theme.textSecondary }}>Created</div>
                  <div className="text-sm font-semibold" style={{ color: theme.text }}>{formatDate(createdAt)}</div>
                </div>
              </div>
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-xs" style={{ color: theme.textSecondary }}>Updated</div>
                  <div className="text-sm font-semibold" style={{ color: theme.text }}>{formatDate(updatedAt)}</div>
                </div>
              </div>
              <div className="flex items-center">
                <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3" style={{ background: hexToRgba(theme.success, 0.1) }}>
                  <User className="w-5 h-5" style={{ color: theme.success }} />
                </div>
                <div>
                  <div className="text-xs" style={{ color: theme.textSecondary }}>Created By</div>
                  <div className="text-sm font-semibold" style={{ color: theme.text }}>User #{createdBy}</div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="mb-6">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <span className="text-xs" style={{ color: theme.textSecondary }}>Theme Color:</span>
                  <div
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs" style={{ color: theme.textSecondary }}>Hover Color:</span>
                  <div
                    className="w-6 h-6 rounded-full border-2"
...
export default ActivityCategoryListCard;